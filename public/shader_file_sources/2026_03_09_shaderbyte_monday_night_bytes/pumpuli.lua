...
function TIC()
	...
	y=H/3
	for i=0,360,2 do
		xx=x+sin(i/(PI/4))*(30+low*3)
		yy=y-cos(i/(PI/4))*(30+low*3)
		ii=abs(i-180)
		--f=ii/80
		f=ft[(i*10)%#ft+1]/200
	...
	for x=0,W do
		f=ft[min((x//2+2),#ft)+1]
		c=1+f//20
		line(x,y,x,y+8,min(15,c))
		for yy=H-H//6+9,H do
			px[x+1][yy+1]=0
		end
	end	
	--cprint("____________",W/2,H-H/6,15,false,5,true)
	cumulate(vbank(),fr,3,0)
	cls()
	vbank(0)
	--[[
	for xx=0,W,STP do
...
function post(fr,st)
	for xx=0,W,st do
			for yy=0,H,st do
				...
				f=vr(x//2,1)
				ox=rnd(-f//40,f//40+1)
				mul(x,y,.95+min(0.05,f*.0005),0,min(3,f//40))--rnd(-1,1),rnd(-3,10)//5)
				add(x,y,-.02)
			end
		end
end
