...
for f=0,num_frames-1 do
    ...
    -- i am now remembering the struggles of the
    -- german keyboard layout....

    label("tst" .. f)
    for a=0, 300 do
        -- crusty amiga demo coder colours innit
        
        c=math.floor(math.sin(a/16+f/16)*8+8)
        c2=math.floor(math.sin(a/16+f/8)*8+8)
        c3=math.floor(math.sin(a/8+f/16)*8+8)
        color(0,(c&15)|(c2&15)<<4|(c3&15)<<8)
        color(8,(c&15)<<8)
        color(10,(c&8)+8<<8)
        color(4,(c&15))
        
        v=math.floor(math.sin(f/4+a/32)*math.sin(f/16+a/16)*4+4)
        
        -- im surprised i remembered the name of the scroll register lol
        push(BPLCON1,(v)|(v)<<4)
        
        push((a&255)<<8|1,0xfffe)
    end
    ...
    for b=1, logo.num_bitplanes do
        push_ptr(reg_offset, "screen+" .. bpl_offset + math.floor(math.sin(f/16)*8+8)*160)
