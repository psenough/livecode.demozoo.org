...
function TIC()
	...
	t=time()/60000*BPM
	flc=fft(0,1024)
	low=fftr(0,128)*.0005*GAIN
	hig=fftr(128,1024)*.0005*GAIN
	tf=tf+low*.1
	ti=t//1
	tfc=t-ti
	tf=ti+tfc^4
	...
	vfft(true)
	for y=0,80,20 do
	--	print("littletheremin",10,10+y,15-y/5,1,2)
	end
	siz=low*5+5
	for i=1,W do
		x=i
	...
	rect(W/2-siz,H/2-siz,siz*2,siz*2,min(15,4+low))
	cumulate(vbank(),fr,2,0,1)
	cls()
	vfft(false)
	cumulate(vbank(),fr,2,1,.8)
	cls()
	vbank(0)
	sx=sx*.2+ax
	sy=sy*.2+ay
	ax=sin(t*.4)*(low*low*.2-hig*.2)
	ay=cos(t*.41)*(low*low*.2-hig*.2)
	drawscr(px,2,sx,sy)
...
--[[	for xx=0,W,STP do
	...
	for x=0,200 do
		f=vqt(x/7*5)*256//1
		txt[x//20+1]=txt[x//20+1].." "..char(min(255,11+f*f*.5)//1)
	end
	for i=0,10 do
		print(txt[i+1],20,10+i*16,15)	
	end
end

function BDR(ii)
	local i=abs(ii*(1+vqt(ii)*.1*(abs(sin(t))*10)*5%4*3*sin(ii/10+t))-50)--abs(H/2-ii+10)
	local fff=vqtsw(i/5+8)+vqtw(i+8)
	
	local odd=i%2==0 and -1 or 1
		local cm={0+(i*.01)+offs*.2+fff*4,1+offs*.2+(ii-10*low-flc)/80,1+i*.01}
	...
	palmix(0,green,white,fff*.1)
	offs=offs+fff*.02
	poke(0x3ff9,.1*-i/8*(1+fff*3*odd*(ii>H/2 and 1 or 0))+fff*8+offs*(1+fff*4)*odd+odd*(ii+40)/H*offs*sin(ii/10+t*10))
...
function post(fr,st)
	for xx=0,W,st do
			for yy=0,H,st do
				...
				ox=rnd(-f//40,f//40)
				oy=low//10*f//1
				mul(x,y,.6+rnd(1,8)/(20-low//4),(ox+1)//2-low//8*(oy>1 and 0 or 1),oy)--rnd(-3,10)//5)
				--add(x,y,-.02)
			end
		end
end

function drawscr(scr,st,ox,oy)
	for xx=0,W,st do
		x=xx+fr%st
		pt=scr[(x+ox//1)%#scr+1]
		for yy=0,H,st do
