...
function TIC()
	...
	for xx=0,W,STP do
		for yy=0,H,STP do
			...
			Y=y/W-.3
			f=ft[y//2%#ft+1]
			X,Y=rot(X,Y,abs(X)+tf*PI/4+Y+f*.01)
			l=X*X+Y*Y
			z=(.2+l+Y)
			di=ditr[(x+y*W)%#ditr+1]
			XX=X/8
			c=((((XX/z+tf/32)*10)%2)<=1) and 1 or 0
				pix(x,y,max(low*max(0,(.51-z)),c*min(15,z*20)+di))
		end
	end
--	vfft(true,15)
	cumulate(vbank(),fr,1,0,2)
	cls()	
	charfft("    test    ",W/2,50+(tfc^.5)*20,15,1,3)
	y=H/2+10
	for x=0,W,.2 do
		f=ft[x//2%#ft+1]
		X=x/W-.5
		line(x,y,x+X*W/8,y+f*10,15)
	end
	cumulate(vbank(),fr,1,1,1)
	cls()
	vbank(0)
	drawscr(px,2)
	charfft("    test    ",W/2,40+(tfc^.5)*20,low,1,3)
