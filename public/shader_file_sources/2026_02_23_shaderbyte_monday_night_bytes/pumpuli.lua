...
function BOOT()
	...
	palstore(0,green)
	palmuladd(1,gray,{.7,.9,.2},{0,30,30},1)
	palstore(1,red)
end
function TIC()
	frm=frm+1
	...
	vbank(1)
	palmuladd(1,gray,{.8,.7,.3},{30,60,80},1)
	palstore(1,green)
	post(fr,2)
	...
	for yy=0,H,STP do
		...
		for i=0,6 do
			X=abs(X)-.07
			Y=abs(Y)-.08
			X,Y=rot(X,Y,(tf/(1+i)/(1+l*abs(_X)))*.4)
		end
		--]]
		
		f=vr(H/2-y/2,1)
		X=X-.2
		X,Y=rot(X,Y,-PI/5+f*.01)
	--	l=lerp(_X*_X+_Y*_Y,X*X+Y*Y,.8)
		fq=abs(X*W)//.9
		c=ft[fq%#ft+1]/40
		--if abs(Y*W)>c*10 then c=0 else c=15 end
		pix(x,y,min(15,max(0,c)))
		p[x+1][y+1]=lerp(px[x+1][y+1]+c,c,.02)
	end
	end
	
	
--	cumulate(vbank(),fr,2,0)
	cls()
	vbank(0)
...
function BDR(i)
	...
	if i>H-H/6-3-30-low and i<H+4 then 
		palmuladd(1,green,{low*.1,1,.997},{2,low,4},1)
		palstore(1,green)
		palset(0,green)
		poke(0x03ff9,(H-i-sin(i/50+tf))*fff)
	else
		poke(0x03ff9,0)
		palmuladd(1,gray,{1,1,1},{0,0,0},1)
		palstore(1,green)
	end
	palmix(0,green,red,fff*.4*(low*low*flc*.01))
end
...
function post(fr,st)
	for xx=0,W,st do
			for yy=0,H,st do
				...
				ox=rnd(-f//40,f//40+1)
				mul(x,y,.83,rnd(-1,1)+ox,rnd(-3,10)//5)
 		--	add(x,y,-.2)
			end
		end
end
...
function add(x,y,a)
	p[x+1][y+1]=max(0,px[x+1][y+1]+a)
end
function mul(x,y,a,ox,oy)
	p[x+1][y+1]=(px[(x+ox)%W+1][(y+oy)%H+1]*a)
	if p[x+1][y+1]<0.001 then p[x+1][y+1]=0 end
end
