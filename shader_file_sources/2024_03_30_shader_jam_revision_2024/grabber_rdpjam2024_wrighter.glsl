...
ivec2 proj_p(vec3 p, float t, out float z){
  ...
  float qt = 2.*t/60*500;
  //p.xy -= 0.5;
  float s = hash_f_s(uint(qt));
  float sb = hash_f_s(uint(qt + 125));
  ...
  */
  ...
  p.xy += sample_disk() * (dof_pow *(0.06 + float(mod(T,4) < 2)*0.02) + 0.0);
...
void main(void){
  ...
  if(gl_FragCoord.x < R.x/14){
    
    if(mod(T,5) < 1.5 && false){
      vec3 p = normalize(sin(vec3(3,2,1)*T*0.1))*2;
      vec3 rd = (hash_v3()*0.2 - p);
      float side = 1.0;
  ...
  col = max(col,0.);
  {
    ...
    #define has2(X) has(has(X.x*35.6 +T*0.0 + 4 + floor(T))*415.5 + has(X.y*32.6+T*0.00)*53.6)
      ...
      for(float i = 0; i < 60; i++){
        ...
        float X = has2(fluv);
        if(X <0.0 * (length(col)*max(sin(T),0))+0.004){
          break;
        }
        tmd /= 2.0;
      }
  ...
  vec2 adv = vec2(
    texture(texPreviousFrame, U/R + vec2(1,0)/R).x - texture(texPreviousFrame, U/R - vec2(1,0)/R).x,
    texture(texPreviousFrame, U/R + vec2(0,1)/R).x - texture(texPreviousFrame, U/R - vec2(0,1)/R).x
  );
  if(floor(mod(T,4))<0.5)
    col -= adv.xyx;
  col = fract(col);
  //col = mix(col,prev.rgb,0.2);
  //for()
