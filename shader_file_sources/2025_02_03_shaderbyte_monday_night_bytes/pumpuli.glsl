...
void main(void)
{
  ...
  zom-=0.5;
  //zom*=0.8+.3*bms-.3*bfl;
  //zom+=vec2(.0002*sin(Gt*8),0);
  //zom*=1;//*uv.x*10;
  zom*=.98;//+sin(Gt)*.2;
  
  rot(zom,(0.02*sin(Gt*PI*.25+(uv.x+uv.y)*10)+pow(bms*.02,4)-pow(bm*.02,8))*bfl*4);
  //zom+=vec2(0.003*cos(Gt*.13),0.007*sin(Gt*.11));
  //rot(zom,.005*sin(Gt)+(PI/8)*(max(0,floor(mod(Gt-7,8)))));
  zom+=0.5;
  ...
  float fx=texture(texFFTSmoothed,.4-abs(c_uv.x*.8)).r*10;

	float f = texture( texFFT, d ).r * 100;
  
  rot(c_uv,ffl*PI/2*floor(mod(Gt*3,3)-1));

  float cir=mod(length(c_uv),2)*(2);
  
  cir=mod(pen(c_uv,ffl*.2),2);
  float cir2=length(c_uv)*(2);
  cir=step(mod(cir-Gt,.2)*3,0.1+pow(bms*.8,3)+f*.005);
  ...
  float cc=tf+abs(uv_.x)+abs(uv_.y)+mod(fGt*.05,2);
  ...
  ci=int(min(max(0,c.r*15),15));
  //rot(c.rg,floor(mod(Gt,3)-1));
  
  c=clamp(c,.0,1.2);
  if(mod((solid.y+Gt*.125)*40,2)>1){
    c*=sharp*-1;
    ci=int(mod(Gt+solid.y*10*floor(solid.x*10+3),16));
  }
  c=mix(c,vec4(palette[ci],0),1-bfl);
  
	out_color = c*(border+prev*2)+sharp;//vec4(c_uv.xy*2,0,1);
