...
ivec2 proj_point(vec3 p,vec3 cameraPostion,mat3 cameraDirection){
  ...
  float fov=0.55;              //FIELD OF VIEW amount
  p.xy/=p.z*fov;              //Perspective projection using field of view above

  //DOF code:
  float dofFocus=200;          //dofFocus is 20 as the camera is 20 unit away (orbit camera has 20 radius), change this to change the focus distance
  float dofScale=0.37;         //scale of dof, how fast it kicks in
  float dofAmount=0.0000005;      //amount of dof
...
void main(void){
  t=fGlobalTime*10;                               //Set global time variable
  seed=0;                                         //Init hash
  ...
  vec3 col = vec3(.4)-length(uv)*0.9;                                    //Background colour and vignette
