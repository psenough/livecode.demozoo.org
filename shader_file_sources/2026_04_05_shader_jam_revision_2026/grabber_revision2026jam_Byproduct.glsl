...
float shape(vec2 p)
{
    ...
    float x = p.x;
    float y = p.y;
    return pow(x*x + y*y - 5.0, 0.5) - x*x*y*y*y;
}
...
void main(void)
{
    ...
    if (hit)
    {
        vec3 p = ro + rd * t;
        vec3 n = getNormal(p);
        vec3 l = normalize(vec3(-0.5, 0.6, -0.4));
        ...
        vec3 base = 0.5 + 0.5 * cos(vec3(0.0, 2.0, 4.0) + fGlobalTime * 0.6 + p.z * 0.3);
