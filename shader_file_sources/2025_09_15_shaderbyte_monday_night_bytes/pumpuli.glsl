...
void main(void)
{
  ...
  float low=texture(texFFTIntegrated,.002).r;
  Gt=pow(low,1.02);
  //Gt=mod(Gt,64);
  float scale=10;
  vec2 ares=vec2(240,136)*scale;
  vec2 reso=ares+10;
  vec2 uv_=uv;
  uv_=floor(uv_*reso);
  uv_-=10;
  ...
  if (uv_.x < 0 || uv_.x > ares.x-10 || uv_.y < 0 || uv_.y > ares.y-10) {
    border = vec4(palette[int(floor(Gt*.25)*2+2+Gtf*1)%16],1);
  }
  uv_-=floor(ares/2)-5;
  uv_/=240;
  ...
  float cir3=length(uv_);
  
  cir=step(cir,(0.25+f*.01)/scale);
  cir2=step(cir2,0.2/scale);
  ...
  for(int i=0; i<80; i++){
    uv=abs(uv)-vec2(0.04+i*.01,0.3*scale);
    uv+=vec2(.2*scale,-.05-i*.005);
    rot(uv,Gt*PI*0.0025-low*.01+.12-i*.2*(mod(i,3)-1));//+uv2.y*.003-uv2.x*.01);
  }
  ln=length(uv);
  float sz=20;
  float fx=texture(texFFTSmoothed,floor(ln*sz)/sz).r*10;
  
	vec4 t = vec4(vec3(mod((uv.x-uv.y+Gt*.05-min(cir,1)/16+fx-ln*10)/scale,1)),0);
	t = clamp( t, 0.0, 1.0 );
  t-=(cir-cir2);
  ...
  ci=ci;
  int ci2=int(cir)*2-int(cir2)*3+int(ci*c*uv.x*2*(.4-ln*2)+ln*10);
  vec4 c2=vec4(0.0);
  c2=cir*2-cir2*3+(ci*c*uv.x*2*(.4-ln*2)+ln*10);
  c=vec4(palette[(int(ci%6/4*2)+ci2%3+min(int(f*uv),3))%16],0);
  c=vec4(step(mod(sin(c2.g/400),1),.5));
  //if(ci%5<2){c=prev*(.8+fx);}
  c*=1+step(mod(log(cir3)-f*.01-low*low*.0001,.9)*scale,(.3-t*.1)*scale)*10*vec4(1,-1,-1,0);
 
  c-=pow(cir3*.17,8)*vec4(1,2,4,0);
  c=clamp(c,0.0,1.0);
  rot(prev.xz,.4);
  if((c.r+c.g+c.b)<.1){c=prev*.8;}
  if((c.r+c.g+c.b)>.9){c-=prev*.7-prev*.9;}
  c=clamp(c,-.1,1.2);
	out_color = vec4(c.rgb,cf);
  //if(border.a==1){out_color=border;}//;+prev*-1;}
}