...
function TIC()
 ...
 fr=dt(frm,STP)
	...
	if low<5 then 
	for i=0,W*H-1,STP do
			ii=i+fr%STP
			subpix(ii,1+dth[ii+1],0,12)
	end
	end
	for xx=0,W,STP do
	for yy=0,H,STP do
		...
		if c>8 then 
			...
			pix(x,y,c-(8*((((max(Y*H,X*H)+t//1*WID/2)//WID+t*4))%2)))
		end
	end
	end
end
...
function BDR(i)
	local f=vqt(H-abs(i-H/2)*2)*2
	f=f*f
	if f>.3 then 
	palmix(0,orange2,white,1*(t%3-2))
	else
	palmix(0,orange2,white,-f-(t%1^4)*.3*lo)
	end
