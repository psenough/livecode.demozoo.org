...
vec4 octree( in vec3 p)
{
  ...
  return vec4(p,pcg3d(idSum+id).x);
}
...
void main(void)
{
  ...
  vec4 oc = octree(vec3(UV,fftTime*3.0));
  ...
  vec3 rayDir = 3 * cameraForward + quadtree_uv.x * cameraLeft + quadtree_uv.y * cameraTop;
  ...
  vec3 diffuse = hue_shift(vec3(1.0,0.5,0.0), rnd);
  
  vec3 shading = ray.isHit ? diffuse * dot(triplanar,abs(ray.normal))*(max(dot(ray.normal, light_direction),0)+0.1) + vec3(blinnPhong) : vec3(0.0);
  
	out_color.rgb += shading;
}