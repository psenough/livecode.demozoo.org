...
float msin(float a)
{
  return sin(a)+sin(a*3.1)*cos(a*2.1);
}
...
//  vec4 d=vec4(length(p)-1-r2*.3,.4,length(p)<3?1:0,.3);
  ...
  float e=length(p.xy);
  e=abs(Rep(e,2))-.3;
  if(S==0) dmin(d,vec4(p.z+1.5,sin(p.x*2)*sin(p.y*2)>0?.4:.2,0,0));          // floor 
  if(S==0) dmin(d,vec4(max(-p.z+1.5,e),sin(p.x*2)*sin(p.y*2)>0?.4:.2,0,0));  // ceil
 
  if(S==1) dmin(d,vec4(p.z+3.5+msin(p.x/3)*msin(p.y+T*30)*cos(T*2)*.1,.6,0,0));          // sea

  vec3 lp=p;
  lp.y+=T*30;
  R(lp.xz,.1);
  R(lp.yz,.1);
  float l=lat(Rep(lp,30))-1;
  float l3=lat(Rep(lp,10))-2;
  R(lp.xz,.1);
  R(lp.yz,.1);
  float l2=lat(Rep(lp,1))-.1;
  dmin(d,vec4(max(15-length(p),max(l,l2)),.6,0,0));          // sea
  ...
  q=abs(p);
  q-=2;
  dmin(d,vec4(length(q.xy)-.01,.4,1+r*2,-.3));  // lasers
  
  if(S==1)
  {
    float pip=abs(length(p.xy)-2.5)-.01;
    float pz=Rep(p.z,3)-10;
    dmin(d,vec4(max(l3,pip),.4,abs(pz)<0?1:0.3,.3));  // pipe
  }
