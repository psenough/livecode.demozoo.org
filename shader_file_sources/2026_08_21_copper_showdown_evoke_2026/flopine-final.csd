...
for i=1, 64 do
    local r = math.floor(9+4*math.cos(tau*i/64))
    local g = math.floor(9+4*math.cos(tau*(i/64 - 1/3)))
    local b = math.floor(9+2*math.cos(tau*(i/64 - 2/3)))
    rain[i] = r << 8 | g << 4 | b
end
...
newlist("setup")
    ...
    color(0, 0xbff)
function copyimg(lab)
    local off = (320 - s0.width)/2 + 320*100  
    blit({
        a = lab,
        d = ptr_add("bp4", off//8),
        func = "a",
        width_words = s0.width // 16,
        height = s0.height,
        amod = s0.height//8,
        dmod = (320 - s0.width)//8
    })
...
for f=0,num_frames-1 do
    ...
    if (f < 25) then
        copyimg("img1")
    elseif (f >= 25 and f < 50) then
        copyimg("img2")
    elseif (f >= 50 and f < 75) then
        copyimg("img3")
    elseif (f >= 75) then
        copyimg("img4")
    end
    
    push_ptr(BPL1PTH, ptr_add("screen", math.floor(f) * line_size) )
    push_ptr(BPL2PTH, ptr_add("screen", math.floor(f+3) * line_size) )
    push_ptr(BPL3PTH, ptr_add("screen", math.floor(f+5) * line_size) )
    push_ptr(BPL4PTH, ptr_add("bp4", line_size * math.floor(math.sin(f/num_frames*math.pi) * 20) ) )
    push_ptr(BPL5PTH, ptr_add("bp5", line_size * math.floor(math.sin(f/num_frames*math.pi) * 50) ))
...
function shader(x,y)
    ...
    local idx = math.floor(x//per)
    local idy = math.floor(y//per)
    
    math.randomseed(idy * 320 + idx)
    local r = math.random()
    
    per = r < 0.5 and per/2 or per
    
    local uvx = x % per - per/2
    local uvy = y % per - per/2

    local size = per/2
    
    local qx = math.abs(uvx) - size
    local qy = math.abs(uvy) - size 
    
    local maxx = math.max(qx, 0)
    local maxy = math.max(qy, 0)
    
    local d = math.abs(math.sqrt(maxx * maxx + maxy * maxy) + math.min(0., math.max(qx, qy)))
    d = math.min(d, math.abs(math.sqrt(uvx*uvx + uvy*uvy) - size/2))
...
label("screen")
bitmap(320, 256*2, 1, false, function(x,y) return shader(x,y) end)

label("bp4")
allocmem(320*256/8, 0)
