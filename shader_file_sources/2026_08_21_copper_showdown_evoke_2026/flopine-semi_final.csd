...
for i=1, 64 do
    local r = math.floor(8+7*math.cos(tau * i/64))
    local g = math.floor(8+7*math.cos(tau * (i/64 - 1/3)))
    local b = math.floor(8+7*math.cos(tau * (i/64 - 2/3)))
    rain[i] = r << 8 | g << 4 | b 
end
...
newlist("setup")
    push({
        ...
        DIWSTRT, 0x2c81,
        DIWSTOP, 0x2cc1,
        BPL1MOD, line_size * (nbpl - 1),
        BPL2MOD, line_size * (nbpl - 1),
        BPLCON0, nbpl << 12,
    })

    local off = (line_size + e_logo_128.width) / 2 + (320*nbpl*100)
    blit({
        a = "img",
        d = ptr_add("display", off//8),
        func = "a",
        width_words = e_logo_128.width // 16,
        height = e_logo_128.height,
        amod = 0,
        dmod = line_size - (e_logo_128.width)//8 + (320*(nbpl-1))//8
    })
...
for f=0,num_frames-1 do
    ...
    for b=0, nbpl-1 do
        local anim = math.floor(math.sin(f/num_frames * math.pi)*(20+b*5))
        push_ptr(BPL1PTH + 4 * b, ptr_add("screen", line_size * nbpl * anim))
    end
    local off = math.floor(math.sin(f/num_frames*math.pi)*5) 
    push_ptr(BPLCON1, off)
    push_ptr(BPL1PTH + 4 * (nbpl - 1), ptr_add("display", off))
...
function shader(x,y)
    ...
    local gax = x % per - per/2
    local gay = y % per - per/2
    local gbx = ((x-per/2) % per) - per/2
    local gby = ((y-per/2) % per) - per/2

    local con = (gax*gax + gay*gay) < (gbx*gbx + gby*gby)
    local uvx = con and gax or gbx
    local uvy = con and gay or gby

    local id = x - uvx

    local d = math.abs(math.sqrt(uvx * uvx + uvy * uvy) - per/3)
    local r = math.fmod(id, 2) == 0 and 6 or 1
    return d < r and 1 or 0
end

label("img")
push(e_logo_128.imem16())

label("screen")
bitmap(320, 256*2, nbpl, true, function(x,y) return shader(x,y) end)

label("display")
for y=nbpl,256, nbpl do
    for x=1, 320, 16 do
        push(0x0)      
    end
end
