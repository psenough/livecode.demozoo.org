...
newlist("setup")
    push({
        ...
        DIWSTRT, 0x2c81,
        DIWSTOP, 0x2cc1,
        BPL1MOD, line_size * (logo.num_bitplanes - 1),
        BPL2MOD, line_size * (logo.num_bitplanes - 1),
...
for f=0,num_frames-1 do
    ...
    lss = {}
    for k, v in ipairs(iss) do
        table.insert(lss, {
            y = (v.y + f) % num_frames,
            x = v.x + math.floor(math.sin(f/num_frames * math.pi*2+v.o) * 16),
            i = v.i,
    ...
    for k,v in ipairs(lss) do
        ...
        push((wy & 0xff) << 8 | 0xdb, 0xfffe)

        
        pos = (sy & 0xff) << 8 | sx
        ctl = (syend & 0xff) << 8 | ((sy >> 8) & 0x1) << 2 | ((syend >> 8) & 0x1) << 1 | v.x & 0x1
        
        push_ptr(SPR0PTH + (SPR1PTH - SPR0PTH) * v.i * 2, "sa" .. f .. "+" .. offset)
        push(SPR0POS + (SPR1POS - SPR0POS) * v.i * 2, pos)
        push(SPR0CTL + (SPR1CTL - SPR0CTL) * v.i * 2, ctl)        

        push_ptr(SPR1PTH + (SPR1PTH - SPR0PTH) * v.i * 2, "sb" .. f .. "+" .. offset)
        push(SPR1POS + (SPR1POS - SPR0POS) * v.i * 2, pos + 8)
        push(SPR1CTL + (SPR1CTL - SPR0CTL) * v.i * 2, ctl)     
    end
...
for y=1,256 do
    for b=1, logo.num_bitplanes do
        push(border)
        push(table.unpack(logo_data, offset, offset + 256//16 -1))
        push(border)
...
function hite(x, y, cx, cy, rx, ry, a)
    cosa = math.cos(a)
    sina = math.sin(a)
    ...
    ax = math.pow(cosa*(x - cx) + sina*(y - cy), 2)
    ay = math.pow(sina*(x - cx) - cosa*(y - cy), 2)
...
function flower(x, y, cx, aa)
    ...
    for n=0, nf-1 do
        ...
        if hite(x, y, fx, fy, fr, fr*4, -a) then
            return 2
        end
    end
...
for f=0, num_frames - 1 do
label("sa" .. f)
bitmap(16, 32, 2, true, function(x, y)
    return flower(x, y, 16, math.pi*2*f/num_frames)
end)

label("sb" .. f)
bitmap(16, 32, 2, true, function(x, y)
    return flower(x, y, 0, math.pi*2*f/num_frames)
end)
end


return resolve(), DMAF_RASTER | DMAF_SPRITE'''
agnus = "ECS 1MiB"
denise = "OCS"
