...
ivec2 proj_p(vec3 p, float t){
  ...
  // depth of field
  p.xy += sample_disk() * pow(abs(p.z - 3.5 + sin(T)*0.5),1.5)*0.008;
...
float map(vec3 p){
    vec3 q = p;
    float d = 10000;
    for(int i = 0; i < 4 + (uint(T/8))%3; i++){
      p = abs(p) - 0.1;
      p.xz *= rot(0.6);
      p.yz *= rot(0.2 + T*0.2 + i + sin(T*0.05));
    }
    d = length(p) - 0.2;
...
void main(void)
{
  ...
  vec3 cam_pos = vec3(hash_f() + 1,0,0);
  ...
  if(hash_f() < 0.5){
    ...
    cam_pos.xz *= rot(0.7 + T*0.002 + floor(T/4));
    cam_pos.xy *= rot(3.7-T*0.002);
    hue = pow(hue,0.05); hue += .7;
    hue += sin(T*0.+sin(T)*4.5 + uv.y)*0.1;
    sat *= 1.;
  ...
  vec3 att = hsv2rgb(vec3(hue*4,sat,1));
  ...
  for(int k = 0; k < 1; k++){
    uv += (hash_v2()*2. - 1.)/R;
    vec3 rd = cam_mat * normalize(vec3(uv*1,fov*40));
    ...
    for(int bnc = 0; bnc < 6; bnc++){
      ...
      for(float i = 0.; i < 50; i++){
        ...
        //d = sign(d) * min(abs(d),0.1);
				if(hash_f()<0. + float(hash_f_s(int(T/4))>0.5)){
					p += rd * side* 0.1;
        ...
        add_to_pixel(pproj, att*(0.05 + fract(T)*0.2));
        if(d < 0.001){
          hit = true;
          ivec2 pproj = proj_p(p,T);
          if(pproj.x >= 0){
